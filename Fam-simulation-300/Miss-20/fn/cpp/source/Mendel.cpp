...
dMatrix<double> Mendel::calPostProbLIK(const dMatrix<double> & lk, const vector<int> & sampleId, double & pedLIK)
{
	if(m_error)
	{
		cout<<"Error in parameters!"<<endl;
	...
	return calPeelingLIK(sampleIndex,pedLIK);
}
...
dMatrix<double> Mendel::calPeelingLIK(const vector<unsigned int> & sampleIndex, double & LIK){
	dMatrix<double> antProb(m_nIndAll,m_nGenotype,-1);
	dMatrix<bool> checkLoop(m_nIndAll,m_nGenotype,false);
	m_loop=false;
	...
	for(size_t i=1;i<likAll.size();i++){
		if(fabs(likAll[i]-likAll[0])>0.0001){
			LIK = -3;
			return dMatrix<double> (1,1,-3);
		}
	}
